...
def test_valid_fetch(mock_get_earnings, authorized_client):
    symbol = Symbol(
        symbol="IBM",
    ...
    mock_get_earnings.return_value = return_value
    payload = {"symbol": "IBM"}
...
@patch("pages.views.overview.financial_data_service.get_earnings")
@pytest.mark.django_db
def test_valid_cache(mock_get_earnings, authorized_client):
    symbol = Symbol(
        symbol="IBM",
        name="International Business Machines Corp",
        type="Stock"
    ...
    response = authorized_client.post(url, payload, format="json")
    assert response.status_code == 200
    assert mock_get_earnings.call_count == 1

# test for invalid case where Alpha Vantage returns error message about rate limit
@patch("pages.views.overview.financial_data_service.get_earnings")
@pytest.mark.django_db
def test_exceeds_rate_limit(mock_get_earnings, authorized_client):
    symbol = Symbol(
        symbol="IBM",
        name="International Business Machines Corp",
        type="Stock"
    ...
    symbol.save()
    mock_get_earnings.return_value = Response(
...
@patch("pages.views.overview.financial_data_service.get_earnings")
@pytest.mark.django_db
def test_symbol_not_in_alpha_vantage(mock_get_earnings, authorized_client):
    symbol = Symbol(
        symbol="IBM",
        name="International Business Machines Corp",
        type="Stock"
