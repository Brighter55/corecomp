import CurrentRatioGraph from "./CurrentRatioGraph.jsx";
...
describe("CurrentRatioGraph", () => {
  beforeEach(() => vi.clearAllMocks());

  test("loads with statement prop", async () => {
    const statement = { annualReports: [{ fiscalDateEnding: "2024-01-01", CurrentRatio: "2.5" }], quarterlyReports: [] };
    mockFilterReports.mockReturnValue(statement.annualReports);
    mockGetPercentChange.mockReturnValue(0);

    render(<MemoryRouter><CurrentRatioGraph statement={statement} period="annually" /></MemoryRouter>);

    expect(await screen.findByText("Current Ratio")).toBeInTheDocument();
  });

  test("updates when statement prop changes", async () => {
    const first = { annualReports: [{ fiscalDateEnding: "2024-01-01", CurrentRatio: "2.5" }], quarterlyReports: [] };
    const second = { annualReports: [{ fiscalDateEnding: "2024-01-01", CurrentRatio: "3.1" }], quarterlyReports: [] };

    mockFilterReports.mockReturnValue(first.annualReports);
    const { rerender } = render(<MemoryRouter><CurrentRatioGraph statement={first} period="annually" /></MemoryRouter>);

    mockFilterReports.mockReturnValue(second.annualReports);
    rerender(<MemoryRouter><CurrentRatioGraph statement={second} period="annually" /></MemoryRouter>);
  ...
  const { container } = render(<MemoryRouter><CurrentRatioGraph statement={[]} period="annually" /></MemoryRouter>);
